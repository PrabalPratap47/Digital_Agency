...
const Navbar = ({theme, setTheme}) => {
    ...
    const [sidebarOpen,setSidebarOpen] = useState(false) // by default sidebar is closed so initially false
...
<div className = {`text-gray-700 dark:text-white sm:text-sm ${!sidebarOpen ? 'max-sm:w-0 overflow-hidden' : 'max-sm:w-60 max-sm:pl-10'}
    ...
    <div className='flex items-center gap-2 sm:gap-4'>
        ...
        <img src={theme === 'dark' ? assets.menu_icon_dark : assets.menu_icon} alt='' onClick={()=> setSidebarOpen(true)} className='w-8 sm:hidden cursor-pointer'/> {/* menu icon for small screen */}
